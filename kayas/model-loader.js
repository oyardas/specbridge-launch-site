(function(){
  ...
  function readProjectData(){
    try{
      ...
      xhr.send(null);
      if(xhr.status===200||xhr.status===0){data=JSON.parse(xhr.responseText);window.KAYAS_PROJECT_DATA=data;}
    }catch(_){data=null;}
  }
  ...
  function replacements(){
    ...
    var pods=c.ic8000Pods||20,perPod=c.itCabinetsPerPod||10,it=c.itCabinetsTotal||200,cdu=c.cduCabinets||6;
    ...
    return {
      ...
      '20 H3C IC8000 pods and 200 total cabinets':pods+' H3C IC8000 pods, '+it+' IT cabinets and '+cdu+' CDU cabinets',
      'The phase-1 concept includes 20 IC8000 pods with 10 cabinets each, targeting 200 cabinets in total. The layout remains fully inside the enclosed data center volume, outside the 5-meter terrace band.':'The represented 3D geometry is the previous concept revision: '+pods+' IC8000 pods with '+perPod+' IT cabinets per pod ('+it+' IT cabinets) plus '+cdu+' auxiliary CDU cabinets. For current investment and capacity decisions, use the Yatırım ve Teknoloji Rehberi baseline: 206 IT cabinets = 196 air-cooled × 7 kW + 10 liquid-cooled × 60 kW = 1.972 MW. The 3D capacity geometry is SUPERSEDED.',
  ...
  function replaceTextNode(node,map){
    ...
    var original=node.nodeValue;if(!original)return;var next=original;
    Object.keys(map).forEach(function(from){if(next.indexOf(from)!==-1)next=next.split(from).join(map[from]);});
    if(next!==original)node.nodeValue=next;
  }
  function walkAndReplace(root){
    if(!root)return;var map=replacements();var walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT,null,false),node;
  ...
  function applyMetrics(){
    var c=capacity();if(!c.itCabinetsTotal)return;
    ...
    var cards=metricGrid.querySelectorAll(':scope > div'),existingCduCard=null;
    Array.prototype.forEach.call(cards,function(card){
      var label=card.querySelector('span'),value=card.querySelector('strong');if(!label||!value)return;
      var text=label.textContent||'';
      if(text.indexOf('Kabinet hedefi')!==-1||text.indexOf('IT cabinet target')!==-1||text.indexOf('IT kabinet hedefi')!==-1){setTextIfChanged(value,String(c.itCabinetsTotal));setTextIfChanged(label,'IT cabinet target · SUPERSEDED 3D revision');}
      if(text.indexOf('IC8000 pod')!==-1){setTextIfChanged(value,String(c.ic8000Pods));setTextIfChanged(label,'IC8000 pod · '+c.itCabinetsPerPod+' IT cabinets/pod');}
      if(text.toLowerCase().indexOf('cdu')!==-1)existingCduCard=card;
    });
    if(existingCduCard){existingCduCard.setAttribute('data-kayas-dynamic','cdu');setTextIfChanged(existingCduCard.querySelector('strong'),String(c.cduCabinets));setTextIfChanged(existingCduCard.querySelector('span'),'CDU cabinets · auxiliary');}
    else{
      var cduCard=document.createElement('div');cduCard.setAttribute('data-kayas-dynamic','cdu');
      cduCard.innerHTML='<strong>'+c.cduCabinets+'</strong><span>CDU cabinets · auxiliary</span>';metricGrid.appendChild(cduCard);
    }
    var duplicates=metricGrid.querySelectorAll('[data-kayas-dynamic="cdu"]');for(var i=1;i<duplicates.length;i++)duplicates[i].remove();
  }
  ...
  function patchSpeech(){
    try{
      ...
      window.speechSynthesis.speak=function(utterance){if(utterance&&typeof utterance.text==='string'){var map=replacements();Object.keys(map).forEach(function(from){if(utterance.text.indexOf(from)!==-1)utterance.text=utterance.text.split(from).join(map[from]);});}return original(utterance);};
