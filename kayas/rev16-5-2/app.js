...
const sun=new THREE.DirectionalLight(0xffffff,2.35);sun.position.set(24,34,94);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);sun.shadow.camera.left=-50;sun.shadow.camera.right=50;sun.shadow.camera.top=75;sun.shadow.camera.bottom=-75;sun.shadow.camera.near=1;sun.shadow.camera.far=110;scene.add(sun);
...
function drawMini(){const c=document.getElementById('mini'),x=c.getContext('2d'),W=c.width=c.clientWidth*devicePixelRatio,H=c.height=c.clientHeight*devicePixelRatio;x.setTransform(devicePixelRatio,0,0,devicePixelRatio,0,0);const w=c.clientWidth,h=c.clientHeight,p=8,s=Math.min((w-2*p)/46,(h-2*p)/110),ox=(w-46*s)/2,oz=(h-110*s)/2;x.fillStyle='#0c1b25';x.fillRect(0,0,w,h);for(const r of ROOMS){x.fillStyle=r.id==='D-02'?'#204a5f':r.id==='A-02'?'#4b3f35':r.id==='B-01'?'#4b4631':r.cat==='terrace'?'#2f4c4b':'#182d39';x.fillRect(ox+r.x*s,oz+r.z*s,r.w*s,r.d*s);x.strokeStyle='#4b6575';x.strokeRect(ox+r.x*s,oz+r.z*s,r.w*s,r.d*s)}if(mode==='walk'){const px=ox+player.x*s,pz=oz+player.z*s;x.fillStyle='#76ddff';x.beginPath();x.arc(px,pz,4,0,Math.PI*2);x.fill();x.strokeStyle='#fff';x.beginPath();x.moveTo(px,pz);x.lineTo(px+Math.sin(yaw)*12,pz+Math.cos(yaw)*12);x.stroke()}x.fillStyle='#dceaf1';x.font='700 9px Segoe UI';x.fillText('N ↑',ox+20*s,oz-2)}
...
document.getElementById('homeBtn').onclick=openLanding;
...
document.getElementById('dataBtn').onclick=()=>{document.getElementById('landing').classList.add('hidden');setWalkPose(27.5,68.2,27.5,56.0,'D-02 · Active IC8000 Data Hall','Architecture-locked procedural cabinet geometry arranged as 10 micro-module corridors. The model represents exactly 196 air-cooled cabinets plus 10 liquid-cooled cabinets; final H3C equipment allocation and auxiliary-cabinet scope remain confirmation required.')};
...
(async()=>{try{overview();requestAnimationFrame(frame);await new Promise(r=>setTimeout(r,250));document.getElementById('loading').classList.add('hidden');document.getElementById('landing').classList.remove('hidden');document.getElementById('status').textContent='SHELL READY · BUILDING ARCH-LOCKED ASSETS';installAssets().then(()=>{document.getElementById('status').textContent='READY · REV16.5.2 ARCHITECTURE LOCKED';}).catch(e=>{console.error(e);document.getElementById('status').textContent='SHELL READY · ASSET FALLBACK MODE';});}catch(e){console.error(e);document.querySelector('#loading .loadbox b').textContent='Rev16.5.2 shell could not initialize';document.querySelector('#loading .loadbox span').textContent=e.message||String(e)}})();
